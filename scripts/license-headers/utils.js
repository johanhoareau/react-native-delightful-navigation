...
function formatHeader(extension) {
	const format =
		CONFIG.commentFormats[extension] || CONFIG.commentFormats.default
	...
	if (format.prefix) {
		return (
			CONFIG.licenseHeader
				.split("\n")
				.map((line) =>
					line.trim() ? `${format.prefix} ${line.trim()}` : format.prefix
				)
				.join("\n") + "\n\n"
		)
	}
...
function removeOldHeaders(content) {
	...
	return patterns.reduce((acc, pattern) => acc.replace(pattern, ""), content)
}
